 ...
 * Wrapper tenant IDs are issued by Hub enrollment, not by schema-sync responses.
 * 
        ...
        try {
            ...
            while (retryCount < maxRetries && !success) {
                try {
                    ...
                    if (is404) {
                        log.warn("Hub could not find tenantId (404). Verify the Hub Agent record, then run dadp wrapper enroll and dadp wrapper refresh.");
                        return false;
                    }
