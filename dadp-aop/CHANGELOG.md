# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

---

## [3.17.1] - 2025-12-12 (배포 전)

### 🎉 릴리즈 정보

**버전**: 3.17.1  
**릴리즈 일자**: 2025-12-12 (최종 업데이트)  
**배포 상태**: ⚠️ **개발 완료, Maven Central 미배포** (배포 전)

### ✅ Changed

- **includeStats 역할 명확화 및 엔진 요청 개선**: `includeStats` 파라미터가 AOP 로깅용으로만 사용되도록 수정하고, 엔진 요청에서 `includeStats` 파라미터를 제거
  - `includeStats=true`일 때 AOP 레벨에서만 상세 로그를 출력 (암복호화 수행, 결과 일부 값)
  - 엔진 요청에서 `includeStats` 파라미터를 완전히 제거 (엔진은 자동으로 통계 수집)
  - AOP에서 `cryptoService.encrypt(data, policy)` 형태로 호출 (includeStats 파라미터 없음)
  - `HubCryptoService`에서 `EncryptRequest`/`DecryptRequest`에 `includeStats` 필드를 설정하지 않음
  - `includeStats`는 AOP 자체의 로깅 기능이며, 엔진 통계 수집과는 무관
  - 엔진 요청 크기 감소 (불필요한 파라미터 제거)
  - 기존 코드 변경 불필요 (동작 방식만 변경)

### ✅ Fixed

- **Stream 반환 타입 read-only 트랜잭션 문제 해결** (2025-12-12)
  - Stream 복호화 시 read-only 트랜잭션에서 UPDATE 쿼리 시도로 인한 에러 해결
  - Stream을 List로 수집한 직후, 복호화 전에 모든 엔티티를 detach하여 Hibernate의 변경 추적 차단
  - `handleStreamDecryption()` 메서드에 EntityManager 파라미터 추가
  - detach된 엔티티는 복호화 후에도 dirty로 마킹되지 않아 read-only 트랜잭션에서 정상 동작
  - 에러 메시지: `Connection is read-only. Queries leading to data modification are not allowed` 해결
  - 영향: `getAllUsersAsStream()`, `getUsersByNameAsStream()` 등 Stream 반환 메서드 정상 동작

### (기타 개발 중)

이 버전은 현재 개발 중이며, 아직 배포되지 않았습니다.

---

## [3.17.0] - 2025-12-09

### 🎉 릴리즈 정보

**버전**: 3.17.0  
**릴리즈 일자**: 2025-12-05  
**배포 일자**: 2025-12-09  
**주요 개선사항**: Engine 직접 연결, 리포지토리 레벨 암복호화 지원, `findAll()` 배치 복호화 최적화 (약 3배 성능 향상)

### ⚠️ 중요: 호환성

**이전 버전과 호환되지 않습니다:**
- **v2.1.0** → **v3.17.0**: 호환성 깨짐 (major 버전 증가)
- **설정 변경 필요**: `hub.crypto.base-url`을 Engine/Gateway URL로 변경
- **코드 변경 권장**: 리포지토리 레벨 암복호화로 전환

### ✅ Added

- **Engine 직접 연결 지원**
  - Hub를 거치지 않고 Engine에 직접 암복호화 요청
  - 성능 향상 (Hub 경유 지연 제거)
  - Gateway URL 지원
- **리포지토리 레벨 암복호화 지원 (권장)**
  - 리포지토리 메서드에 `@Encrypt`/`@Decrypt` 어노테이션 적용
  - 서비스 레이어와 리포지토리 레이어 분리
  - 일관성 있는 암복호화 처리
- **`findAll()` 배치 복호화 최적화**
  - `CryptoService.batchDecrypt()` 메서드 추가
  - `HubCryptoService.batchDecrypt()` 메서드 추가
  - 개별 복호화 대비 약 3배 이상 성능 향상 (1000건 조회 시: 7.15초 → 2.31초)
- **배치 암호화 지원**
  - `CryptoService.batchEncrypt()` 메서드 추가
  - `HubCryptoService.batchEncrypt()` 메서드 추가
  - 하나의 객체 내 여러 필드 암호화 시 배치 처리
- **알림 기능 통일**
  - `dadp-hub-crypto-lib`의 `HubNotificationClient` 사용
  - 알림 정책 통일
- **환경변수 통일**
  - `DADP_CRYPTO_BASE_URL` (필수)
  - `DADP_HUB_BASE_URL` (선택, 알림용)

### 🔧 Changed

- **설정 변경**
  - `hub.crypto.base-url`: Hub URL → Engine/Gateway URL
  - `hub.crypto.api-base-path`: `/hub/api/v1` → `/api` (Engine) 또는 `/hub/api/v1` (Hub)
- **테스트 앱 개선**
  - 개별 테스트 옵션 제거 (`findById` 기반 개별 조회 테스트 제거)
  - `findAll`/`saveAll` 통일
  - 불필요한 코드 제거 (`getAllUserIds()` 등)
  - 프론트엔드 UI 정리

### 🔧 Fixed

- **AOP 복호화 문제 해결**
  - 리포지토리 메서드에 어노테이션 적용으로 복호화 정상 동작
- **성능 최적화 검증 완료**
  - `findAll()` 배치 복호화 성능 측정 및 분석 완료

### ⚠️ Deprecated

- **서비스 레벨 암복호화**
  - 리포지토리 레벨 암복호화 사용 권장
  - 향후 버전에서 제거 예정

### 📝 Known Issues

- **`saveAll()` 배치 암호화 미지원**
  - Spring Data JPA의 구조적 제약으로 인해 개별 암호화만 지원
  - 각 엔티티별로 개별 암호화 API 호출 발생
- **`@Query` 네이티브 쿼리 미지원**
  - 향후 지원 예정
- **`Page<T>`, `Slice<T>` 반환 타입 미지원**
  - 향후 지원 예정

---

## [2.1.0] - 2025-11-06

### 🎉 릴리즈 정보

**버전**: 2.1.0  
**릴리즈 일자**: 2025-11-06  
**주요 개선사항**: Maven Central 배포 완료, Hub 암복호화 라이브러리 통합

### ✅ Added

- **Maven Central 배포 완료**
  - Group ID: `io.github.daone-dadp`
  - Artifact ID: `dadp-aop`, `dadp-aop-spring-boot-starter`
- **Hub 암복호화 라이브러리 통합**
  - `dadp-hub-crypto-lib` 의존성 추가
- **Spring Boot Starter 패키지**
  - 자동 설정 및 의존성 관리

### 🔧 Changed

- **Group ID 변경**
  - `com.dadp` → `io.github.daone-dadp`
- **Maven Central 배포**
  - JitPack에서 Maven Central로 전환

---

## [2.0.0] - 2025-10-17

### 🎉 릴리즈 정보

**버전**: 2.0.0  
**릴리즈 일자**: 2025-10-17  
**주요 개선사항**: 초기 릴리즈, AOP 라이브러리 완전 구현

### ✅ Added

- **AOP 라이브러리 완전 구현**
  - `dadp-aop`, `dadp-aop-spring-boot-starter` 모듈 완성
  - Spring Auto-Configuration 지원
- **어노테이션 지원**
  - `@Encrypt`, `@Decrypt`, `@EncryptField`, `@DecryptField`
- **EncryptionAspect AOP 구현**
  - 메서드 입출력/필드 단위 암복호화 인터셉트
- **Hub 암복호화 라이브러리 통합**
  - `HubCryptoService` 자동 초기화 기능

---

**참고**: 이전 버전(v1.x)은 내부 개발 버전으로 공개되지 않았습니다.

