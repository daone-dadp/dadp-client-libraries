 ...
 * Handles JDBC wrapper schema collection and manual schema synchronization.
 * 
...
public class JdbcSchemaSyncService {
     ...
     */
    public boolean syncSchemaToHub(String tenantId, String alias, Long currentVersion) {
        return schemaSyncService.syncSchemaToHub(tenantId, alias, currentVersion);
    }
    ...
    public boolean syncSpecificSchemasToHub(List<SchemaMetadata> schemas) {
        ...
        // 영구저장소에서 tenantId 로드
        String tenantId = loadTenantIdFromStorage();
        
        if (tenantId == null || tenantId.trim().isEmpty()) {
            log.warn("Cannot perform schema sync: tenantId not available.");
            return false;
        }
        ...
        try {
            ...
            if (synced) {
                log.info("Specific schemas sent: tenantId={}, schemaCount={}", tenantId, schemas.size());
            }
    ...
    private String loadTenantIdFromStorage() {
        ...
        if (tenantIdManager != null) {
            ...
            return proxyConfig.getTenantId();
        }
    }
}
