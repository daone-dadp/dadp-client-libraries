...
**배포 버전 → 5.5.0 (현재 작업 중인 버전) — #배포 전**

> 이 문서는 **마지막 배포 버전(5.1.0)** 릴리즈와 **5.5.0** 작업 내용을 통합한 릴리즈 문서입니다.  
> 배포를 실행하면 이 문서를 **배포 릴리즈 문서**로 픽스(배포 일자·배포 완료 상태 반영)합니다.
...
**이전 문서 기준**: 5.1.0  
**주요 개선사항**: 동적 SELECT 복호화 스킵 이슈 해결, instanceId당 부팅 1세트 공유, Root CA TrustStore 자동 설치(설계), HTTPS 통신 지원(설계), Hub 5.5.0 연동
