...
public class DadpAopProperties {
     ...
     * - application.properties: dadp.hub-base-url
     * 
    ...
    public static class AopConfig {
         ...
         * 
         * 설정 옵션:
         * - "jpa": JPA 기반 감지 (EntityManagerFactory 필요)
         * - "reflection": 리플렉션 기반 감지 (JPA 없이도 동작)
         * - "annotation": 어노테이션 스캔 기반 감지
         * - "auto": 자동 선택 (JPA가 있으면 JPA, 없으면 Reflection)
         * 
         * 기본값: "auto"
         * 환경변수: DADP_AOP_ENTITY_DETECTOR_TYPE
         */
        private String entityDetectorType = "auto";
        
        /**
         * 엔티티 스캔 기본 패키지
         * 
         * reflection 또는 annotation 타입 사용 시 스캔할 기본 패키지 지정
         * null이면 전체 클래스패스 스캔
         * 
         * 기본값: null (전체 스캔)
         * 환경변수: DADP_AOP_ENTITY_SCAN_BASE_PACKAGE
         */
        private String entityScanBasePackage;
        ...
        public String getEntityDetectorType() {
            // 환경 변수 DADP_AOP_ENTITY_DETECTOR_TYPE 우선 사용
            String envType = System.getenv("DADP_AOP_ENTITY_DETECTOR_TYPE");
            if (envType != null && !envType.trim().isEmpty()) {
                return envType.trim().toLowerCase();
            }
            // 설정 파일에서 읽은 값 사용
            if (entityDetectorType != null && !entityDetectorType.trim().isEmpty()) {
                return entityDetectorType.trim().toLowerCase();
            }
            // 기본값: "auto"
            return "auto";
        }
        
        public void setEntityDetectorType(String entityDetectorType) {
            this.entityDetectorType = entityDetectorType;
        }
        
        public String getEntityScanBasePackage() {
            // 환경 변수 DADP_AOP_ENTITY_SCAN_BASE_PACKAGE 우선 사용
            String envPackage = System.getenv("DADP_AOP_ENTITY_SCAN_BASE_PACKAGE");
            if (envPackage != null && !envPackage.trim().isEmpty()) {
                return envPackage.trim();
            }
            // 설정 파일에서 읽은 값 사용
            return entityScanBasePackage;
        }
        
        public void setEntityScanBasePackage(String entityScanBasePackage) {
