 ...
 * 정책 매핑 동기화 오케스트레이터
 * 
 * Manages wrapper runtime refresh.
 * - unchanged: no work
 * - changed: refresh policy bindings, endpoint, and runtime options
 * - missing enrollment: log and wait for the manual CLI enroll and refresh flow
 * 
...
public class PolicyMappingSyncOrchestrator {
     ...
     * 생성자
     * 
     * @param tenantIdManager TenantId 관리자
    ...
    /**
     * MappingSyncService 업데이트 (tenantId 변경 시 호출)
     * 
     ...
     * 
     * Flow:
     * 1. Require CLI-issued tenantId/enrollment.
     * 2. Check whether Hub runtime refresh changed.
     * 3. Apply policy mappings, endpoint, and runtime options.
     */
    public void checkMappingChange() {
        if (!tenantIdManager.hasTenantId()) {
            log.warn("No tenantId found. Run dadp wrapper enroll, then dadp wrapper refresh.");
            return;
        }
        ...
        try {
            ...
            // Hub에서 버전 변경 여부 확인
            boolean hasChange = mappingSyncService.checkMappingChange(currentVersion);
